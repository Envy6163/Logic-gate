...
{
    [SerializeField] private string levelsStateRelativeFilePath = "LevelsState.json";
    private string LevelsStateFilePath => Path.Combine(Application.persistentDataPath, levelsStateRelativeFilePath);
    ...
    public void SaveLevelState(int levelIndex,bool state)
    {
        LevelsStateSaveData data;
        if (File.Exists(LevelsStateFilePath))
        {
            data = GetLevelsState();
        }
        else
        ...
        data[levelIndex] = state;
        File.WriteAllText(LevelsStateFilePath, JsonUtility.ToJson(data, true));
    }
    public void SaveNumberOfLevelsUnlocked(int num)
    {
        if (num > GetNumberOfLevelsUnlocked())
            File.WriteAllText(LevelsStateFilePath, num.ToString());
    }

    public int GetNumberOfLevelsUnlocked()
    {
        if (File.Exists(LevelsStateFilePath))
            return Convert.ToInt32(File.ReadAllText(LevelsStateFilePath));
        return 1;
    }
    public LevelsStateSaveData GetLevelsState()
    {
        if (File.Exists(LevelsStateFilePath))
        {
            return JsonUtility.FromJson<LevelsStateSaveData>(File.ReadAllText( LevelsStateFilePath));
        }
        return null;
    }
    public void Save()
